				...
				if ($(this).attr("useMultColor") == "true") {
					if ($(this).attr("useCheckBox") == "false") {//如果checkBox为false采用普通多选模式
						...
						if($(this).find("th").length>0){
							var $checkIcon=$('<img src='+prePath+'icon/checkAllOff.gif title="点击全选" class="hand"></span>');
							...
							if ($(this).attr("headFixMode") == "true"){
								$checkIcon.toggle(function(){
									...
									$(this).attr("src",prePath+'icon/checkAllOn.gif');
									...
									$(this).attr("src",prePath+'icon/checkAllOff.gif');
